...
# Welburn and Strong (2022) benchmarks (manuscript Table 3):
#   initial_loss = phi0 (direct cost), systemic_loss = M (aggregate systemic loss).
# The calibration matches the simulated indirect loss (sum of phi_f, i.e. M_hat - phi0)
# to the benchmark indirect loss  M - phi0 = sum(phi_f); see find_optimal_beta.
#   finance     : M = 3781,  phi0 = 56,  -> target sum(phi_f) = 3725
#   information : M = 20659, phi0 = 308, -> target sum(phi_f) = 20351
WELBURN_STRONG = {
    "finance":     {"initial_loss": 56,  "systemic_loss": 3781,  "beta_initial": 0.000606},
...
def calibrate(sector, num_simulations=DEFAULT_NUM_SIMULATIONS, seed=0,
              N=N, gamma_bar=GAMMA_BAR, T=T):
    """Calibrate the beta distribution for one sector and save it to .npy.

    N, gamma_bar and T default to the manuscript values (1000, 0.1, 10) but can
    be overridden to re-calibrate beta under a different network size, baseline
    operational resilience or calibration horizon.
    """
    ...
    for i in range(num_simulations):
        beta_values[i] = find_optimal_beta(
            target_loss, beta_initial=params["beta_initial"], gamma_bar=gamma_bar,
            initial_loss=initial_loss, N=N, T=T)
        if (i + 1) % 1000 == 0:
            print(f"[calibration] {sector}: {i + 1}/{num_simulations}")

    out = os.path.join(calibration_dir(), f"beta_{sector}.npy")
...
def run(sectors=("finance", "information"), num_simulations=DEFAULT_NUM_SIMULATIONS, seed=0,
        N=N, gamma_bar=GAMMA_BAR, T=T):
    return {s: calibrate(s, num_simulations=num_simulations, seed=seed,
                         N=N, gamma_bar=gamma_bar, T=T) for s in sectors}
...
def _parse(argv=None):
    ...
    p.add_argument("--gamma-bar", type=float, default=GAMMA_BAR,
                   help="baseline operational resilience for calibration (default: 0.1)")
    p.add_argument("--T", type=int, default=T, help="calibration horizon in days (default: 10)")
...
def main(argv=None):
    args = _parse(argv)
    run(sectors=tuple(args.sectors), num_simulations=args.num_simulations, seed=args.seed,
        N=args.N, gamma_bar=args.gamma_bar, T=args.T)
